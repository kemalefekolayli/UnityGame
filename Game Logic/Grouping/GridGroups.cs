...
public class GridGroups
{
    ...
    {
        CubeColor targetType = gridStorage.GetColorAt(startPos);

        // If not a valid colored cube (r, g, b, y), return empty list
        if ((int)targetType == 99 ||
            (targetType != CubeColor.r && targetType != CubeColor.g &&
             targetType != CubeColor.b && targetType != CubeColor.y))
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (Vector2Int dir in directions)
            {
                ...
                CubeColor adjacentType = gridStorage.GetColorAt(adjacentPos);
    ...
    {
        CubeColor type = gridStorage.GetColorAt(position);

        // If not a colored cube (r, g, b, y) then not groupable
        if ((int)type == 99 ||
            (type != CubeColor.r && type != CubeColor.g &&
             type != CubeColor.b && type != CubeColor.y))
            return false;
        ...
        for (int y = 0; y < gridHeight; y++)
        {
            for (int x = 0; x < gridWidth; x++)
            {
                ...
                if (processedPositions.Contains(position) ||
                    (type != CubeColor.r && type != CubeColor.g && type != CubeColor.b && type != CubeColor.y))
                    continue;
